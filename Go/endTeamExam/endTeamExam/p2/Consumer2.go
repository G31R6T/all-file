...
func main() {
	...
	go func() {
		for d := range msgs2 {
			bodyString := string(d.Body)
			fmt.Println("Received direct broadcast message:", bodyString)
			d.Ack(false)
		}
	}()
	...
	go func() {
		for d := range msgs3 {
			bodyString := string(d.Body)
			fmt.Println("Received fanout broadcast message:", bodyString)
			d.Ack(false)
		}
	}()
	...
	q4, err := ch.QueueDeclare(
		"p2SharedQueue", // name,,empty string let server generate id
		false,     // durable
		true,      // delete when unused
		false,     // exclusive
		false,     // no-wait
