...
/**1
  Write a function, that takes a list of functions, and a list of tuples (Int, Int) where the first Int indicates which function to use and the second Int acts as a parameter and returns a list of the results.
...
/**2
  Write a function that takes a list of integers and returns a list of result integers based on how many integers were in the parameter list.
...
/**5
  ...
  The dot product of two vectors can be computed as: < xa, xb, xc, ...> * < ya, yb, yc, ...> = (xa*ya) + (xb*yb) + (xc*yc) + ...
...
/**6
   Given a list of characters, split it into a tuple in which the first part only contains digits ('0'..'9'), the second part contains the rest. */
...
/**8
  ...
  Find the 'unique' right triangle in the list eg. (3,4,5) and (4,3,5) are the same triangle, only one will appear in the answer list [(3,4,5),(4,3,5)] -> [(3,4,5)] */
